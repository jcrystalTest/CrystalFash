...
export class ManagerProduct{
	...
	**/
	static filterProductsBySize(base : NetworkBase,size:Size,onSuccess: (r:ProductNormal[])=> void ,onError : (error : RequestError)=>void = defaultOnError){
		let params:string = null;
		let headers =  new HttpHeaders({'Content-Type': 'application/json'});
		let ruta:string = "http://localhost:8080/api/product/filterProductsBySize";
		if (base.onError) {
			onError = base.onError;
		}
		if(AppConfiguration.DEBUG)console.log("GET "+ruta);
		if (size !== null && typeof(size) !== 'undefined') {
			params = (params===null?"?":(params + "&")) + "size=" + String(size.id);
		}
		if (params) {
			ruta+=params;
		}
	...
	**/
	static getCategories(base : NetworkBase,onSuccess: (r:string[])=> void ,onError : (error : RequestError)=>void = defaultOnError){
		let params:string = null;
		let headers =  new HttpHeaders({'Content-Type': 'application/json'});
		let ruta:string = "http://localhost:8080/api/product/getCategories";
		if (base.onError) {
			onError = base.onError;
		}
		if(AppConfiguration.DEBUG)console.log("GET "+ruta);
	...
	**/
	static getProductById(base : NetworkBase,id:number,onSuccess: (r:ProductNormal)=> void ,onError : (error : RequestError)=>void = defaultOnError){
		let params:string = null;
		let headers =  new HttpHeaders({'Content-Type': 'application/json'});
		let ruta:string = "http://localhost:8080/api/product/getProductById";
		if (base.onError) {
			onError = base.onError;
		}
		if(AppConfiguration.DEBUG)console.log("GET "+ruta);
	...
	**/
	static getProducts(base : NetworkBase,onSuccess: (r:ProductNormal[])=> void ,onError : (error : RequestError)=>void = defaultOnError){
		let params:string = null;
		let headers =  new HttpHeaders({'Content-Type': 'application/json'});
		let ruta:string = "http://localhost:8080/api/product/getProducts";
		if (base.onError) {
			onError = base.onError;
		}
		if(AppConfiguration.DEBUG)console.log("GET "+ruta);
